...
- `test-hooks.sh` goes from 45 to 55 assertions. Nine cover the write test, including the two-repos-one-session case; one is the executable-bit check picking up the new hook file. Three mutations were run against the new logic and each was killed: dropping the write test (3 failures), not consuming the marker (4), and failing open without a `session_id` (5).
