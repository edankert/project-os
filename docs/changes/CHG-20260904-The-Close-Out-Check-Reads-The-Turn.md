...
commit: ""
pr: ""
impacts: ["tools/adapters/claude-code/hooks/close-out-check.sh", "tools/adapters/claude-code/hooks/session-touch.sh", "tools/adapters/claude-code/hooks/shared/session-marker.sh", "tools/adapters/claude-code/hooks.json", "tools/instructions/HOOKS.md", "tools/adapters/claude-code/ADAPTER.md", "tools/scripts/test-hooks.sh"]
issues: []
features: []
...
- **The shared helper lives in `hooks/shared/`, not `hooks/lib/`.** `obsidian-supernote-sync/.gitignore` carries a bare `lib/`, which matches a directory of that name at any depth, so the marker helper would never have been committed there — and a repo without it falls back to blocking every stop, which is safe but is not the fix. `shared/` is clean across all twelve repos, checked rather than assumed.
- `test-hooks.sh` goes from 45 to 74 assertions. Nine cover the write test, including the two-repos-one-session case; one is the executable-bit check picking up the new hook file. Nine more are new: the disk check added for ISS-0055 was not enough, because a repo with `core.fileMode = false` records a new hook as `100644` whatever its mode on disk — which is how `session-touch.sh` was added `100644` an hour after that fix landed. The harness now also asserts the **index** mode of every tracked hook, and the same mutation confirms it fails. Ten more assert that no hook file is gitignored, using `check-ignore --no-index`, because plain `check-ignore` reports nothing for an already-tracked file and would have passed in the one repo that had not committed it yet. Three mutations were run against the new logic and each was killed: dropping the write test (3 failures), not consuming the marker (4), and failing open without a `session_id` (5).
