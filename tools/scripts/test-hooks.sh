...
for pair in "empty:$h_empty" "planning:$h_plan" "doing:$h_doing"; do
  name="${pair%%:*}"; text="${pair#*:}"
  check "hint, $name state: no review sentence" "$(hasnot "$text" 'independent-reviewer'; echo $?)" "$text"
done

# Size bound (TASK-0103): at most 3 lines and 600 characters, so the hint never
# grows into the SessionStart slice FEAT-0021 serves.
for pair in "empty:$h_empty" "planning:$h_plan" "doing:$h_doing" "review:$h_rev"; do
  name="${pair%%:*}"; text="${pair#*:}"
