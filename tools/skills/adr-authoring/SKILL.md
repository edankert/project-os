...
3. **If the decision is a quantified rule** (*every member of DOMAIN satisfies P*), it is a rule-ADR:
   - **Name the domain first.** If the enumerable set cannot be named, **stop** — the rule is not ready to be decided, and no amount of drafting will make it ready. Often the registry that would enumerate it is the real first deliverable.
