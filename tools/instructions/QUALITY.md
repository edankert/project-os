...
- Do not mark an issue `fixed`, its single terminal status (ADR-0008), unless its verifying tests are `passing`. Verification lives in the linked `[[test]]`, where it carries evidence.
- Do not mark a requirement `implemented`, its terminal status (ADR-0007), unless every acceptance criterion is ticked with evidence or reconciled; an unresolved criterion on a terminal requirement is a build failure (validator REQ-BOXES). There is no `verified` requirement status.
  - **A requirement is never gated on linked tests**; test status is informational for it, recorded in the criterion's evidence. Reason: gating on tests would penalise linking a test at all (ADR-0007). The test gate still applies to tasks, issues and features.
  - Do set `implemented` when it is due: a requirement left at `draft`/`approved` after its feature is terminal is a build failure (validator REQ-STALE).
...
- Do not mark a feature `done` unless every task in its `tasks:` list is scope-resolved (`done` or `cancelled`) and required tests are `passing`. `deferred` never resolves scope: descope the task through the deferral procedure (`STATUSES.md`, "Deferral and re-adoption"); never flip it to `done` or drop it from the list (validator VERIFY and DEFER).
- Do not mark a feature `done` while any requirement naming it in `implements:` has an unresolved acceptance criterion (validator FEATURE-REQ): satisfy, reconcile or descope the requirement first. Enforced forward-only; how that is implemented is stated once in `STATUSES.md` ("Grandfathering").
