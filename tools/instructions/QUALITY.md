...
- Do not mark a feature `done` while any requirement that names it in `implements:` still has an unresolved acceptance criterion (validator FEATURE-REQ). Satisfy the criteria, reconcile them, or descope the requirement (`deferred`/`cancelled`/`superseded`) first. A feature closing over its own unbuilt requirements is the failure this gate exists to stop; it is enforced **forward-only**, so features that closed before ADR-0007 landed are not retroactively invalidated.
  - How "forward-only" is implemented is stated once, in `STATUSES.md` ("Grandfathering").
...
- **The gate is keyed on a status, not on a note being touched**: a `TST-*` reaching `passing`, a requirement reaching `implemented`, a feature reaching `done`. Each requires an independent review pass per `../skills/independent-review/SKILL.md` — a **clean context**: a session that starts from the notes and the diff alone, never the author's reasoning trace, and is never the session that authored the work. A human pass also satisfies this and remains the strongest option.
...
- **A `CHG-*` note does not owe a review (ADR-0019).** It records what happened; what a reviewer can usefully challenge is the change itself, and that is caught at the three gates above while the work is live. Reviewing the note months later reviews the prose. Measured before the decision: 206 REVIEW findings fleet-wide, 87 in one repo, none discharged in six months — an obligation nobody meets is a countdown, not a standard. Reviewing a change note remains possible and is never wrong; it stops being owed.
- **An acceptance test does not owe a review either, for a different reason.** It rests at `active` and never reaches `passing`, because its verdict is `mark:` and its lifecycle is `draft`/`active`/`retired` (`STATUSES.md` `[[test]]`). **The review of an acceptance test is the walk** — a person performing it and recording what they saw is exactly what an independent pass would be asked to supply, so gating it would ask for the same evidence twice. This survives ADR-0031 unchanged: it was true of the `check` type by construction and it is true of `level: acceptance` by the same construction, which is why the merge did not need an exemption to preserve it.
- *That first bullet was ambiguous until 2026-08-18 (ISS-0196): this file said the gate applied to "any change that creates or updates a `TST-*`" while `TESTING.md` said it was keyed on the status, and the validator implemented the status reading. Under ADR-0031 the difference stopped being academic — the note-touched reading would have made several hundred migrated acceptance tests gate-bearing overnight. The status reading is the rule.*
