...
- **A `CHK-*` check does not owe a review either, for a different reason.** A check is not a `TST-*`: it never reaches `passing`, because its verdict is `mark:` and its lifecycle status is `draft`/`active`/`retired` (`STATUSES.md` `[[check]]`). **The review of a check is the walk** — a person performing it and recording what they saw is exactly what an independent pass would be asked to supply, so gating it would ask for the same evidence twice.
